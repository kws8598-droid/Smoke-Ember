...
export default function RecipeView({ recipe }: { recipe: Recipe }) {
  ...
  const r = bySlug.get(recipe.slug) ?? recipe;
  const more = related(r).map((item) => bySlug.get(item.slug) ?? item);
  const dessert = r.protein === "desserts";
      ...
      <section className="relative min-h-[52vh] overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 pb-10 pt-28">
          <Link href={dessert ? "/desserts" : "/recipes"} className="text-sm text-parchment/80 hover:text-cream">
            ← {dessert ? "All desserts" : "All recipes"}
          </Link>
          ...
          {r.ingredients?.map((g, gi) => (
            <div key={`${g.group}-${gi}`} className="mt-6">
              ...
              <ul className="mt-2 space-y-2">
                {g.items.map((item, ii) => (
                  <li key={`${item}-${ii}`} className="flex gap-2 text-parchment/90">
                    <input type="checkbox" className="mt-1 accent-ember" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
          ...
          <ol className="mt-6 space-y-8">
            {r.steps?.map((s, i) => (
              <li key={`${s.title}-${i}`} className="grid grid-cols-[auto_1fr] gap-4">
                ...
                <div>
                  <h3 className="text-xl text-cream">{s.title}</h3>
                  <p className="mt-2 whitespace-pre-wrap text-parchment/85">{s.body}</p>
                </div>
              </li>
            ))}
          </ol>
