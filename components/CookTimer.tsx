...
export default function CookTimer({ slug, milestones }: { slug: string; milestones?: Milestone[] }) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) { const p = JSON.parse(raw); setStarted(p.started ?? null); setDone(p.done ?? []); }
    } catch {}
  }, [key]);
  useEffect(() => {
    if (!started) return;
  ...
  function persist(nextStarted: number | null, nextDone: string[]) {
    localStorage.setItem(key, JSON.stringify({ started: nextStarted, done: nextDone }));
  }
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-bark p-5">
      ...
      <div className="mt-4 flex gap-2">
        <button onClick={() => { const t = Date.now(); setStarted(t); persist(t, done); }} className="rounded-full bg-ember px-4 py-2 text-sm text-cream hover:bg-ember-hot">{started ? "Restart" : "Start cook"}</button>
        {started && <button onClick={() => { setStarted(null); persist(null, done); }} className="rounded-full border border-white/15 px-4 py-2 text-sm">Clear</button>}
      </div>
