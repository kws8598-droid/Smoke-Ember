...
const chips = [{ id: "all", label: "Anything" }, { id: "beef", label: "Beef" }, { id: "pork", label: "Pork" }, { id: "poultry", label: "Poultry" }, { id: "fish", label: "Fish" }, { id: "venison", label: "Venison" }, { id: "mutton", label: "Mutton" }, { id: "gator", label: "Gator" }, { id: "sides", label: "Sides" }];
export default function TonightPicker() {
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-ink/70 p-5 backdrop-blur-md shadow-2xl">
      ...
      <div className="mt-3 flex flex-wrap gap-2">{times.map((t) => (
        <button key={t.id} onClick={() => setTime(t.id)} className={`h-11 rounded-full px-4 text-sm ${time === t.id ? "bg-cream text-ink" : "bg-ash text-parchment"}`}>{t.label}</button>
      ))}</div>
      <div className="mt-3 flex flex-wrap gap-2">{chips.map((p) => (
        <button key={p.id} onClick={() => setProtein(p.id)} className={`h-11 rounded-full px-3 text-xs ${protein === p.id ? "bg-ember/20 text-ember" : "text-parchment/80 hover:text-cream"}`}>{p.label}</button>
      ))}</div>
