...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: recipe
      ? { images: [{ url: `/food/${recipe.slug}?v=9`, alt: recipe.title }] }
      : undefined,
  };
}
...
export default async function RecipePage({
  ...
  return (
    <main className="page">
      ...
      <img className="hero-photo" src={`/food/${recipe.slug}?v=9`} alt={recipe.title} />
          ...
          <div className="grid">
            {related.map((item) => (
              <Link className="card" key={item.slug} href={`/recipes/${item.slug}`}>
                <img className="card-photo" src={`/food/${item.slug}?v=9`} alt={item.title} />
